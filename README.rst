...
by `Dhall`_, but it is less rigidly typed and therefore much more flexible.
...
as well as `null` and the two booleans `true` and `false`.
...
Object keys are treated as literal values, not as variables.
...
All the above structures work equivalently in objects, exception that keys must
be provided::

    let buildobj = (x) => {a: 1, if x > 3: x: x, c: 3}
    in buildobj(4)          # => {a: 1, x: 4, c: 3}

You can use the built-in `items` function to produce an object like this::

    let buildobj = (list) => {for [key, val] in items: key: val}
    in buildobj([["a", 1], ["b", 2]])
...
The function `add` can be used from another file like this::
    ...
    let mylib = import("mylib.gold")
    in mylib.add(1, 2)

or, more idiomatically, like this::

    let {add} = import("mylib.gold")
    in add(1, 2)
